...
//		解耦,降低代码和代码之间的耦合度 可以直接通过 接口或者是抽象父类就可以获取到实体对象
     ...
     * 加载敌人
     */
    public static void loadEnemy(int leveltype) {
        for (int i = 0; i < 5; ) {
            boolean flag = true; //碰撞的标志
            //获取其他坦克和地图的信息，判断是否相互碰撞
            List<ElementObj> maps = em.getElementsByKey(GameElement.MAPS);
            List<ElementObj> enemys = em.getElementsByKey(GameElement.ENEMY);
            Enemy enemy = (Enemy) new Enemy().createElement("");
            for (int j = 0; j < maps.size(); j++) {
                ElementObj map = maps.get(j);
                if (enemy.pk(map)) {
                    flag = false;
                    enemy = null;
                    break; //如果生成的地方是墙壁等地图元素，则重新生成
                }
            }
            for (int k = 0; flag && k < enemys.size(); k++) {
                ElementObj hasEnemy = enemys.get(k);
                if (enemy.pk(hasEnemy)) {
                    flag = false;
                    enemy = null;
                    break; //如果生成的地方是墙壁等地图元素，则重新生成
                }
        ...
        for (int i = 0; i < leveltype; ) {
            boolean flag = true; //碰撞的标志
            //获取其他坦克和地图的信息，判断是否相互碰撞
            List<ElementObj> maps = em.getElementsByKey(GameElement.MAPS);
            List<ElementObj> enemyBs = em.getElementsByKey(GameElement.ENEMYB);
            EnemyB enemyB = (EnemyB) new EnemyB().createElement("");
            for (int j = 0; j < maps.size(); j++) {
                ElementObj map = maps.get(j);
                if (enemyB.pk(map)) {
                    flag = false;
            ...
            for (int k = 0; flag && k < enemyBs.size(); k++) {
                ElementObj hasEnemy = enemyBs.get(k);
                if (enemyB.pk(hasEnemy)) {
                    flag = false;
                    enemyB = null;
                    break; //如果生成的地方是墙壁等地图元素，则重新生成
                }
            ...
            if (flag) {
                ...
                em.addElement(enemyB, GameElement.ENEMYB);
                i++;
            }
        }
    }
